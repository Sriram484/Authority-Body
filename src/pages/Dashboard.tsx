...
const Dashboard: React.FC<DashboardProps> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    // it's okay not to depend on carouselImages here – we just want a simple loop
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const stats = [
    {
      label: t("dashboard.stats.totalAgencies"),
    ...
    {
      label: t("dashboard.stats.pendingCertificates"),
      value: data.certificateRequests.filter((r: any) => r.status === "Pending")
        .length,
      icon: FileCheck,
    ...
    {
      label: t("dashboard.stats.pendingClaims"),
      value: data.courseClaimRequests.filter((r: any) => r.status === "Pending")
        .length,
      icon: Clock,
  ...
  const recentActivity = [
    ...data.certificateRequests
      ...
      .slice(0, 3)
      .map((r: any) => ({
        type: t("dashboard.activity.types.certificateRequest"),
        title: `${r.studentName} - ${r.courseTitle}`,
        date: r.submittedDate,
        status: t("dashboard.activity.status.pendingReview"),
      })),
    ...data.courseClaimRequests
      .filter((r: any) => r.status === "Pending")
      .slice(0, 2)
      .map((r: any) => ({
        type: t("dashboard.activity.types.courseClaim"),
